    ...
    <div class="py-10">
      <div class="container mx-auto px-4">
        ...
        <div v-else-if="coursesStore.detailedCourse" class="bg-gray-800 rounded-xl p-8">
          <h1 class="text-3xl font-bold text-white mb-4">{{ coursesStore.detailedCourse.title }}</h1>
          <p class="text-gray-300 mb-6">{{ coursesStore.detailedCourse.description }}</p>
          
          <div class="flex items-center mb-6">
            <img :src="coursesStore.detailedCourse.instructor.avatar" :alt="coursesStore.detailedCourse.instructor.name" class="w-12 h-12 rounded-full mr-4">
            <div>
              <p class="text-white font-medium">{{ coursesStore.detailedCourse.instructor.name }}</p>
          ...
          <div class="mb-8">
            ...
            <ul class="space-y-2">
              <li v-for="(lesson, index) in coursesStore.detailedCourse.lessons" :key="index" class="flex items-center text-gray-300">
          ...
          <button class="bg-primary hover:bg-primary/90 text-white font-bold py-3 px-6 rounded-lg transition-colors duration-300">
            Enroll Now
          </button>
        </div>
