  ...
  <form class="mt-8 space-y-6" @submit.prevent="handleSubmit">
    ...
    <div class="rounded-md shadow-sm space-y-6 pt-4">
      <div>
        ...
        <input id="username" v-model="form.username" name="username" type="text" autocomplete="username" required
          ...
          :class="{ 'border-red-500': getError('username') }" placeholder="Username" @blur="handleFieldBlur('username', form.username)" />
      ...
      <div>
        ...
        <input id="email" v-model="form.email" name="email" type="email" autocomplete="email" required
          class="bg-[#2A2A2A] text-white placeholder-gray-400 border border-gray-600 focus:border-primary focus:ring-primary appearance-none rounded-lg relative block w-full px-3 py-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 focus:z-10 sm:text-sm"
          :class="{ 'border-red-500': getError('email') }" placeholder="Email" @blur="handleFieldBlur('email', form.email)" />
      ...
      <div>
        ...
        <p class="text-gray-400 text-xs mt-1">Password must be at least 6 characters with uppercase, lowercase, and number</p>
      </div>
      ...
      <div>
        ...
        <input id="confirmPassword" v-model="form.confirmPassword" name="confirmPassword" type="password"
          ...
          class="bg-[#2A2A2A] text-white placeholder-gray-400 border border-gray-600 focus:border-primary focus:ring-primary appearance-none rounded-lg relative block w-full px-3 py-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 focus:z-10 sm:text-sm"
          :class="{ 'border-red-500': getError('confirmPassword') }" placeholder="Confirm Password"
    ...
    <div class="flex items-center">
      <input id="terms" v-model="form.termsAccepted" name="terms" type="checkbox"
        ...
        :class="{ 'border-red-500': getError('termsAccepted') }" @blur="handleFieldBlur('termsAccepted', form.termsAccepted)" />
    ...
    <p v-if="getError('termsAccepted')" class="text-red-500 text-sm mt-1">{{ getError('termsAccepted') }}</p>
...
const signUpSchema = z.object({
  username: z.string().min(VALIDATION_LIMITS.USERNAME_MIN, AUTH_ERRORS.USERNAME_TOO_SHORT).max(VALIDATION_LIMITS.USERNAME_MAX).regex(VALIDATION_PATTERNS.USERNAME, AUTH_ERRORS.USERNAME_INVALID_FORMAT),
  email: z.string().email(AUTH_ERRORS.EMAIL_INVALID).max(VALIDATION_LIMITS.EMAIL_MAX),
  password: z.string().min(VALIDATION_LIMITS.PASSWORD_MIN, AUTH_ERRORS.PASSWORD_TOO_WEAK).max(VALIDATION_LIMITS.PASSWORD_MAX).regex(VALIDATION_PATTERNS.PASSWORD, AUTH_ERRORS.PASSWORD_TOO_WEAK),
  confirmPassword: z.string().min(1, AUTH_ERRORS.CONFIRM_PASSWORD_REQUIRED),
  termsAccepted: z.boolean().refine(value => value === true, { message: AUTH_ERRORS.TERMS_NOT_ACCEPTED })
...
const { form, errors, isValid, isFormValid, validateField, validateAll, getError, reset } = useZodValidation<SignupFormData>(
  signUpSchema,
  {
    username: '',
    email: '',
  ...
  try {
    ...
    if (!result.success) {
      ...
      if (result.error?.includes(SHARED_AUTH_ERRORS.USERNAME_OR_EMAIL_EXISTS)) {
        ...
        errors.value.confirmPassword = SHARED_AUTH_ERRORS.CONFIRM_PASSWORD_REQUIRED || 'Confirm password is required'
      } else {
