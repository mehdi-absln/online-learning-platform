...
describe('LessonPage', () => {
  ...
  it('shows error state when lesson is not found', async () => {
    // The page derives its error state from `courseError` (the error returned
    // by useLesson/useCourse), so we drive it via the mocked course error ref.
    courseErrorRef.value = 'Lesson could not be loaded.'
