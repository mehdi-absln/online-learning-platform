    ...
    <div class="rounded-md shadow-sm space-y-6 pt-4">
      <div>
        ...
        <input id="username" v-model="form.username" name="username" type="text" autocomplete="username" required
          ...
          :class="{ 'border-red-500': errors.username }" placeholder="Email or username" @blur="validateUsername" />
...
import type { SigninFormData, AuthResponse, SignInFormErrors } from '~/types/types'
...
const handleSubmit = async () => {
  ...
  if (!result.success) {
    ...
    errors.password = result.error || 'Sign in failed'
  } else {
    // Sign in successful
