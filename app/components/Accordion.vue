      ...
      <h3 class="m-0">
        ...
        >
          <div class="text-left">
            ...
            <div v-if="item.duration" class="text-sm text-gray-600 mt-1">{{ item.duration }}</div>
          </div>
          <svg
      ...
      >
        <div v-if="item.lessons && item.lessons.length > 0" class="space-y-2">
          <div
            ...
            :key="lessonIndex"
            class="flex items-center p-2 hover:bg-gray-50 rounded group"
          >
            <svg
            ...
            <div class="flex-1">
              ...
              <span v-if="lesson.duration" class="text-xs text-gray-500 ml-2"
...
interface Emits {
  ...
  'lesson-click': [payload: { sectionIndex: number; lessonIndex: number; lesson: CourseContentLesson }]
}
...
const openItems = ref<boolean[]>(props.items.map((_, index) => index === props.modelValue))

// Check if an accordion item is open
const isOpen = (index: number) => openItems.value[index]

// Toggle the accordion item at the specified index
const toggleAccordion = (index: number) => {
  if (props.exclusive) {
    // If exclusive mode, close all and open only the clicked item
    openItems.value = props.items.map((_, i) => i === index)
  } else {
  ...
  const openIndex = openItems.value.findIndex((item) => item)
